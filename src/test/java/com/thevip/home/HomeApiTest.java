...
class HomeApiTest {
    ...
    void 홈_응답은_참여자수와_완료여부가_반영된_전체_항목을_내려준다() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/home").with(loginAs("30001")))
                ...
                .andExpect(jsonPath("$.data.todaySchedule[0].menuType").value("VOTE"))
                .andExpect(jsonPath("$.data.todaySchedule[0].title").value("인기가요 생방송 투표"))
                .andExpect(jsonPath("$.data.todaySchedule[0].platformNames").value(org.hamcrest.Matchers.contains("하이어(Higher)")))
                .andExpect(jsonPath("$.data.todaySchedule[1].menuType").value("MUSIC"))
                .andExpect(jsonPath("$.data.todaySchedule[1].title").value("오늘 저녁 8시 30분 멜론 개별곡 다운로드 총공"))
                .andExpect(jsonPath("$.data.todaySchedule[1].platformNames.length()").value(2))
                .andExpect(jsonPath("$.data.todaySchedule[1].platformNames[0]").value("멜론"))
                .andExpect(jsonPath("$.data.todaySchedule[1].platformNames[1]").value("벅스(Bugs)"))
                .andExpect(jsonPath("$.data.todaySchedule[1].detailId").exists())
